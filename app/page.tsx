...
type Trend = { rank:number; platform:string; title:string; pattern:string; views:string; likes:string; score:number; growth:string; difficulty:string; hook:string; fit:string; steps:string[]; source?:string; creator?:string; publishedAt?:string; evidence?:string };
...
export default function Home(){
  ...
  useEffect(()=>{const stored=localStorage.getItem('economize-saved-trends');if(stored){try{const parsed=JSON.parse(stored);if(Array.isArray(parsed)&&parsed.every(item=>typeof item==='string'))setSaved(parsed);else{localStorage.removeItem('economize-saved-trends');setSaved([])}}catch{localStorage.removeItem('economize-saved-trends');setSaved([])}}},[]);
  ...
  async function runSearch(more=false){
    ...
    }catch(reason){const message=reason instanceof Error?reason.message:'Falha inesperada na pesquisa.';if(activeBrandRef.current===requestBrand){setError(message);setSearched(true)}else setBrandSessions(current=>({...current,[requestBrand]:{...current[requestBrand],error:message,searched:true}}))}finally{setLoadingByBrand(current=>({...current,[requestBrand]:false}))}
  }
  ...
  return <main className={brand==='SAFE-K'?'trend-app safe-k':'trend-app'}>
    <aside className="sidebar">
      ...
      <button className="new" onClick={()=>{setQuery('');setSearched(false)}}>＋ Nova pesquisa</button>
    ...
    <section className="workspace">
      <header><div className="online"><i/> Pesquisa ampla · adaptação para {brand}</div><button>GS</button></header>
      <div className="content">
        ...
        {view!=='report'&&searched&&!loading&&<section className="results">
          ...
          <div className={live?'demo live':'demo'}><b>{live?'✓':'i'}</b><p><strong>{live?'Pesquisa real concluída:':'Modo de demonstração:'}</strong> {live?'Os resultados abaixo vieram da web. Abra a fonte e confira as métricas antes de produzir.':'Os exemplos e números abaixo são ilustrativos até a chave da API ser configurada.'}</p></div>
          <div className="list">{visible.map(t=>{const expanded=open===t.rank;return <article className={expanded?'card expanded':'card'} key={t.rank}>
            ...
            {expanded&&<div className="details"><div className="detail-grid"><div><small>GANCHO OBSERVADO</small><p>“{t.hook}”</p></div><div><small>SINAL DE CRESCIMENTO</small><p className="growth">↗ {t.growth}</p></div><div className="wide"><small>COMO ADAPTAR — SEM COPIAR</small><p>{t.fit}</p></div>{t.evidence&&<div className="wide"><small>EVIDÊNCIA E LIMITAÇÕES</small><p>{t.evidence}</p></div>}</div>{t.source?<a className="source-link" href={t.source} target="_blank" rel="noreferrer">Assistir ao vídeo original ↗</a>:<span className="source-pending">Link disponível somente na pesquisa real</span>}<div className="steps"><small>ESBOÇO PARA GRAVAÇÃO</small><ol>{t.steps.map(s=><li key={s}>{s}</li>)}</ol></div><div className="actions"><span className={`difficulty ${t.difficulty==='Fácil'?'easy':t.difficulty==='Médio'?'medium':'hard'}`}>● {t.difficulty} de gravar</span><button onClick={()=>toggleSave(t.rank)}>{saved.includes(savedKey(t.rank))?'✓ Salvo':'◇ Salvar'}</button><button className="primary" onClick={()=>setScript(t)}>Criar roteiro completo →</button></div></div>}
          </article>})}</div>
